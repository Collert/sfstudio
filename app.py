...
def call_sick(id):
    ...
    if not pas:
        flash("Не знайдено абонемента зв'язаного з цим профілем")
...
def book_private_class(addon):
    ...
    if request.method == "GET":
        if addon == "4":
            ...
            title = f"Дієтологічна консультація з {session['first']}"
        elif addon == "3":
            belts = db.session.query(Belt).filter(Belt.coach == True).all()
            title = f"1:1 тренування з {session['first']}"
        else:
        ...
        flash(f"Запит надіслано тренеру: {coach.first} {coach.last}")
...
def booking_response(id, response):
    ...
    if agree:
        ...
        flash("Ви погодилися провести це заняття")
        return redirect(f"/event/{event_out.id}")
    else:
        event.accepted = False
        event.reason = request.form.get("reason")
        db.session.commit()
        flash("Ви відмовилися проводити це заняття")
